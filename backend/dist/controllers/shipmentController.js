...
exports.updateFreightDetails = exports.updateShipmentLocation = exports.updateShipmentStatus = exports.getShipmentById = exports.getAllShipments = exports.createShipment = void 0;
const db_1 = __importDefault(require("../db"));
const createShipment = async (req, res) => {
    try {
        const { customerId, shipmentType, direction, mode, origin, destination, consigneeName, consigneeAddress, shipperName, shipperAddress, commodity, grossWeight, netWeight, volume, packagesCount, containerNumber, containerSize, carrier, vesselName, flightNumber, vehicleNumber, etd, eta, } = req.body;
        const count = await db_1.default.shipment.count();
        const shipmentNumber = `GXA-SH-${10000 + count + 1}`;
        const shipment = await db_1.default.$transaction(async (tx) => {
...
const updateShipmentStatus = async (req, res) => {
    try {
        ...
        const oldShipment = await db_1.default.shipment.findUnique({ where: { id: parseInt(id) } });
        if (!oldShipment) {
            return res.status(404).json({ message: 'Shipment not found' });
        }
        ...
        const shipment = await db_1.default.shipment.update({
            where: { id: parseInt(id) },
            data: updateData,
        });
        // Log activity
...
const updateFreightDetails = async (req, res) => {
    try {
        const { id } = req.params;
        const { carrier, vesselName, flightNumber, vehicleNumber, containerNumber, containerSize, etd, eta, } = req.body;
        const shipment = await db_1.default.shipment.update({
            where: { id: parseInt(id) },
            data: {
                carrier,
...
exports.updateFreightDetails = updateFreightDetails;
